...
"""SM80-compatible DSA indexer logits.

The GLM-5/DeepSeek DSA index cache is stored as E4M3FN plus one FP32 scale
per token.  Ampere cannot execute E4M3 tensor-core instructions, so this
backend decodes the stored bytes to BF16 and uses BF16 tensor cores.  The
kernel structure is adapted from vLLM's architecture-independent DSA
fallback, but the wrappers below implement SGLang's packed 64-token page
layout and ragged ``[row_start, row_end)`` contract.
"""
...
def fp8_paged_mqa_logits_triton(
    ...
    Args:
        ...
        context_lens:  [B] or [B, 1] int32. Multi-token speculative
            decode uses a different per-query contract and is rejected.
        block_tables:  [B, max_blocks] int32
        max_model_len: output width. This must remain constant when the call
            is captured by a CUDA graph; eager callers may use the active max.
    ...
    if next_n != 1:
        raise ValueError(
            "The initial SM80 SGLang backend supports one decode token per "
            f"request, got next_n={next_n}. Disable speculative decoding."
        )
    if context_lens.ndim == 2:
        if context_lens.shape != (B, 1):
            raise ValueError(
                "Expected context_lens [B] or [B, 1] for non-speculative "
                f"decode, got {tuple(context_lens.shape)}."
            )
        context_lens = context_lens.squeeze(1)
    if context_lens.shape != (B,):
        raise ValueError(
            f"Expected context_lens shape {(B,)}, got {tuple(context_lens.shape)}"
        )
    if q.dtype != torch.float8_e4m3fn:
        raise ValueError(f"Expected E4M3FN query, got {q.dtype}")
    if head_dim != 128:
        raise ValueError(f"The DSA indexer head dimension must be 128, got {head_dim}")
    if max_model_len <= 0:
...
def fp8_mqa_logits_triton(
    ...
    if q.dtype != torch.float8_e4m3fn or k_fp8.dtype != torch.float8_e4m3fn:
        raise ValueError(
            "The CUDA DSA index cache must use E4M3FN; got "
            f"q={q.dtype}, k={k_fp8.dtype}."
        )
    if head_dim != 128 or k_fp8.shape != (N, head_dim):
        raise ValueError(
            "Expected DSA q [M, H, 128] and k [N, 128], got "
            f"q={tuple(q.shape)}, k={tuple(k_fp8.shape)}."
        )
    if weights.shape != (M, num_heads):
        raise ValueError(
            f"Expected weights {(M, num_heads)}, got {tuple(weights.shape)}"
        )
    if k_scales.shape != (N,):
        raise ValueError(f"Expected k scales {(N,)}, got {tuple(k_scales.shape)}")
    if cu_seqlen_ks.shape != (M,) or cu_seqlen_ke.shape != (M,):
        raise ValueError("Ragged row starts/ends must both have shape [M].")
